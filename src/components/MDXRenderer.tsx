...
const MDXRenderer: React.FC<MDXRendererProps> = ({ content }) => {
  ...
  processedContent = processedContent.replace(
    /<YouTube\s+videoId="([^"]+)"[^>]*\/>/g,
    (match, videoId) => {
      return `<div class="youtube-embed-container my-6">
        <iframe 
          width="100%" 
          height="400" 
  ...
  // Standardized handling for all heading levels with archivo font and consistent spacing
  processedContent = processedContent.replace(/^# (.*$)/gim, '<h1 class="text-3xl md:text-4xl font-bold mb-4 mt-6 font-archivo">$1</h1>');
  processedContent = processedContent.replace(/^## (.*$)/gim, '<h2 class="text-2xl md:text-3xl font-bold mb-3 mt-5 font-archivo">$2</h2>');
  processedContent = processedContent.replace(/^### (.*$)/gim, '<h3 class="text-xl md:text-2xl font-bold mb-3 mt-4 font-archivo">$1</h3>');
  ...
  // Lists with consistent formatting and reduced spacing
  ...
  // Blockquotes with consistent styling and spacing
  processedContent = processedContent.replace(/^> (.*$)/gim, '<blockquote class="pl-4 border-l-4 border-muted italic my-4 py-1">$1</blockquote>');
  ...
  processedContent = processedContent.replace(
    ...
    (match, text) => {
      ...
      if (text.trim().length > 0 && !text.includes('<')) {
        return `<p class="mb-3 font-roboto leading-relaxed">$1</p>`;
      }
      return match;
    }
  );
  ...
  return (
    <div className="markdown-content prose prose-lg max-w-none dark:prose-invert prose-headings:font-archivo prose-p:font-roboto prose-p:mb-3 prose-p:leading-relaxed prose-pre:p-0 prose-pre:leading-tight">
